function duplicate() {
  ...
  for (i = 0; i < numRows; i++) {
    ...
    for (var l = 0; l < types; l++) {
      ...
      if (!check) {
        ...
        forms[2] = "=IFERROR(M" + (row+i) + "/K" + (row+i) + ',"N/A")';
        forms[3] = "=IFERROR(M" + (row+i) + "/J" + (row+i) + ',"N/A")';
        forms[4] = "=IFERROR(R" + (row+i) + "/Q" + (row+i) + ',"N/A")';
        forms[5] = "=IFERROR(T" + (row+i) + "/R" + (row+i) + ',"N/A")';
        forms[6] = "=IFERROR(T" + (row+i) + "/Q" + (row+i) + ',"N/A")';
        forms[7] = "=IFERROR(X" + (row+i) + "/J" + (row+i) + ',"N/A")';
        
        if (l == 0) {
          rows[i] = [range[i][0], range[i][1], forms[0], '', range[i][2], range[i][4], forms[1],
                     range[i][5], forms[2], forms[3], '', range[i][6], range[i][8], forms[4], range[i][9],
                     forms[5], forms[6], '', range[i][3], forms[7], range[i][7]];
        } else if (l == 1) {
          rows2[i] = [range2[i][0], range2[i][1], forms[0], '', range2[i][2], range2[i][4], forms[1],
                     range2[i][5], forms[2], forms[3], '', range2[i][6], range2[i][8], forms[4], range2[i][9],
                     forms[5], forms[6], '', range2[i][3], forms[7], range2[i][7]];
        }
      } else if (check == true) {
  ...
  while (!check) {
    ...
    if (input.getSelectedButton() == ui.Button.OK) {
      date = input.getResponseText().replace("-","/").replace("-","/");
      
      if (date.indexOf('/') == -1 || date.split('/').length != 3) {
        ui.alert('Error!', 'The date must be divided by a "/". Please follow the format "MM/DD/YY"', ui.ButtonSet.OK);
      } else {
        date = date.split('/');
        if (date[0].length != 2 || parseInt(date[0]) < 1 || parseInt(date[0]) > 12) {
          ...
          input2 = ui.alert('Year Confirmation', 'The year you entered (' + date[2] + ') is not the current year (' + curYear.substring(2) +
            '). Is this the year you meant to enter?', ui.ButtonSet.YES_NO_CANCEL);
          
          if (input2 == ui.Button.YES) { check = true; ss.toast('Check', 'true', 3); return; }
