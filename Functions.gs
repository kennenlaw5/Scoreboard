...
function moveCA() {
  ...
  while (!check) {
    ...
    if (caName.getSelectedButton() == ui.Button.CANCEL) { return; }
    for (i = 0; i < teamCAs.length; i++) {
      if (caName.getResponseText().toLowerCase() == teamCAs[i][0].toLowerCase()) { check = true; initial_team = teamStart + i; }
    }
    if (!check) {
  ...
  while (!check) {
    for (var i = 0; i < teams.length && !check; i++) {
      if (end_team.getResponseText().toLowerCase() == teams[i].toLowerCase()) { 
        ...
        else { end_team = end_team[i] + finalTeamSize - 1; endWasLastTeam = true; }
      }
    }
    if (!check) { 
